...
public class FireCommand extends Command {
    ...
    public boolean execute(Robot target) {
        ...
        for (RobotHandler robotHandler: Server.currentWorld.getRobotHandlersAsList()){
            ...
            if (Server.currentWorld.getRobotHandlersAsList().size()>1 && !target.getName().equalsIgnoreCase(robotName)){
        ...
        try(FileWriter fileWriter = new FileWriter(file+jsonFilePath)){
            ...
            new StdFontColor().printTextColor("Could not write to HitOrMiss "+ e, "error");
        }
